...
class exploEvi(models.Model):
    # 炸药及原材料案件物证表
    case_ID = models.CharField('案件编号', max_length=10)
    evidenceID = models.CharField('物证编号', max_length=10)
    user = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name='处理人员')
    inputDate = models.DateTimeField('录入日期', default=datetime.now)
    eviState = models.CharField('物证状态', max_length=30, null=True, blank=True)
    eviMake = models.CharField('物证制备方法', max_length=30, null=True, blank=True)
    eviDraw = models.CharField('物证提取方法', max_length=30, null=True, blank=True)
    eviAnalyse = models.CharField('物证分析方法', max_length=30, null=True, blank=True)
    analyseCondition = models.CharField('分析条件', max_length=30, null=True, blank=True)
    picUrl = models.ImageField('炸药物证外观图片路径', upload_to="image/exploEvi/", null=True, blank=True)
...
class exploChEvi(models.Model):
    ...
    炸药及原材料案件物证子元素表
    """
    exploEviFile = models.ForeignKey(exploEviFile, verbose_name=u"对应的物证文件", related_name="exploChEvi",
...
class exploEviPeak(models.Model):
    ...
    炸药及原材料案件物证峰值表
    """
    exploEviFile = models.ForeignKey(exploEviFile, verbose_name=u"对应的物证文件", related_name="exploEviPeak",
                                     on_delete=models.CASCADE)
...
class devCompEvi(models.Model):
    ...
    picUrl = models.ImageField(max_length=100, upload_to="image/devCompEvi/", null=True, blank=True,
                               verbose_name="装置物证外观图片路径")
    picDescrip = models.CharField(max_length=100, null=True, blank=True, verbose_name="图片描述")
    note = models.CharField(max_length=200, null=True, blank=True, verbose_name="备注")
...
class devCompEviFile(models.Model):
    ...
    devCompEvi = models.ForeignKey(devCompEvi, verbose_name=u"对应的物证", related_name="devCompEviFile",
                                   on_delete=models.CASCADE)
    user = models.ForeignKey(User, verbose_name=u"处理人员", on_delete=models.CASCADE)
    detectDevice = models.CharField(max_length=30, null=True, blank=True, verbose_name="检测设备名称及型号")
    detectMrfs = models.CharField(max_length=20, null=True, blank=True, verbose_name="仪器厂家")
...
class devCompChEvi(models.Model):
    ...
    爆炸装置案件物证成分子元素表
    """
    devCompEviFile = models.ForeignKey(devCompEviFile, verbose_name=u"对应的物证文件", related_name="devCompChEvi",
                                       on_delete=models.CASCADE)
    detectType = models.CharField(max_length=20, null=True, blank=True, verbose_name='检测类型')
    elementsList = models.CharField(max_length=300, null=True, blank=True, verbose_name='元素列表')
...
class devCompEviPeak(models.Model):
    ...
    爆炸装置案件物证成分峰值表
    """
    devCompEviFile = models.ForeignKey(devCompEviFile, verbose_name=u"对应的物证文件", related_name="devCompEviPeak",
                                       on_delete=models.CASCADE)
    peakPos = models.FloatField(default=0, verbose_name="峰高位置")
    peakHeight = models.FloatField(default=0, verbose_name="峰高")
    peakArea = models.FloatField(default=0, verbose_name="峰面积")
